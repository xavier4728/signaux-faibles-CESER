...
CATEGORIES = [
    ...
    "Aménagement du territoire", "Gouvernance", "Économie", "Social",
    "Transport", "Énergie", "Numérique", "Autre",
]
...
def main():
    ...
    logger.info(f"Catégorisation de {len(items)} préconisations par LLM (batch={BATCH_SIZE})...")
    ...
    for start in range(0, len(items), BATCH_SIZE):
        ...
        logger.info(f"  Batch {start // BATCH_SIZE + 1}/{(len(items) + BATCH_SIZE - 1) // BATCH_SIZE} ok.")
    ...
    if "Autre" in category_counts and category_counts["Autre"] and not any(s["category"] == "Autre" for s in category_stats):
        category_stats.append({"category": "Autre", "regions": category_counts["Autre"]})
    ...
    for i in range(n):
        for j in range(n):
            if i == j:
                ...
                overlap = sum(min(vi.get(c, 0), vj.get(c, 0)) for c in set(vi) | set(vj))
